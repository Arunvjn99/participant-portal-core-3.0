...
import { ContributionLayoutV2 } from "../components/ContributionLayoutV2";
import { ENROLLMENT_V2_STEP_PATHS } from "../config/stepConfig";
import { useCoreAIModalOptional } from "../../context/CoreAIModalContext";

/** V2 Contribution — UI from ContributionLayoutV2; state from useEnrollment + useContributionStore. Read-only by default; Edit unlocks controls, Done saves and locks. */
export function ContributionPage() {
  ...
  const handleDoneContribution = useCallback(() => {
    const draft = loadEnrollmentDraft();
    if (draft) {
      saveEnrollmentDraft({
        ...draft,
    ...
    >
      <div className="flex flex-col gap-6 pb-12">
        <ContributionLayoutV2
          ...
          isEditingContribution={isEditingContribution}
          onEditContribution={() => setIsEditingContribution(true)}
          onDoneContribution={handleDoneContribution}
