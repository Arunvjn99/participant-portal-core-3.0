  ...
  return (
    <ScenarioShell user={user} accentColor="#8b5cf6">
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
        <h3 className="font-semibold text-[var(--color-text)]">Income Distribution Plan</h3>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-[var(--color-success)]/20 bg-[var(--color-success)]/10 p-5">
          <h3 className="font-semibold text-[var(--color-success)]">Portfolio Health</h3>
          ...
          <div className="mt-4 h-2 rounded-full bg-[var(--color-success)]/20">
            <div className="h-2 w-full rounded-full bg-[var(--color-success)]" />
          </div>
          <p className="mt-1 text-xs text-[var(--color-success)]">Sustainability: Excellent</p>
        </div>

        <div className="rounded-xl border border-violet-200 bg-violet-50 p-5">
          <h3 className="font-semibold text-violet-900">Quick Actions</h3>
...
function IncomeRow({ label, amount, pct, color }: { label: string; amount: string; pct: string; color: string }) {
  return (
    <div className="rounded-lg border border-[var(--color-border)] p-3">
      ...
      <p className="text-xs text-[var(--color-textSecondary)]">{pct}</p>
    </div>
  );
}
...
function ActionButton({ label }: { label: string }) {
  return (
    <button
      type="button"
      className="rounded-lg border border-violet-300 bg-[var(--color-surface)] px-4 py-2 text-sm font-medium text-violet-700 transition-colors hover:bg-violet-100"
    >
      {label}
    </button>
  );
