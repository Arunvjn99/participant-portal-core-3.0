  ...
  const insights = [
    {
      ...
      coreAiPrompt: `Enrollment review AI Recommendation: ${t("enrollment.insightIncreasePreTax")}. ${t("enrollment.insightIncreasePreTaxDesc")}`,
    },
    {
      icon: (
      ...
      coreAiPrompt: `Enrollment review AI Recommendation: ${t("enrollment.insightTargetDate")}. ${t("enrollment.insightTargetDateDesc")}`,
    },
  ];
    ...
    <>
      ...
      >
        <div className="flex flex-col gap-6">
          <EnrollmentAiHint
            titleKey="aiSystem.enrollmentReviewHintTitle"
            bodyKey="aiSystem.enrollmentReviewHint"
            prompt="I'm on the final enrollment review before submit. What should I double-check in my plan, contributions, and investments?"
          />
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
            ...
            <div className="xl:col-span-4 xl:sticky xl:top-24 flex flex-col gap-6">
              ...
              <div className="mt-4 space-y-3 rounded-2xl border p-4 ai-recommendation" style={{ borderColor: "var(--ai-border)" }}>
                <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: "var(--ai-primary)" }}>
                  {t("aiSystem.aiRecommendation")}
                </p>
                {insights.map((row) => (
                  <div key={row.title} className="rounded-xl border border-[var(--enroll-card-border)] bg-[var(--enroll-card-bg)] p-3">
                    <div className="flex gap-2">
                      <span className="text-[var(--enroll-brand)]">{row.icon}</span>
                      <div className="min-w-0">
                        <p className="text-xs font-bold" style={{ color: "var(--enroll-text-primary)" }}>{row.title}</p>
                        <p className="mt-0.5 text-[11px] leading-relaxed" style={{ color: "var(--enroll-text-secondary)" }}>{row.description}</p>
                        <p className="mt-1 text-[10px] font-semibold" style={{ color: "var(--enroll-accent)" }}>{row.impact}</p>
                        <button type="button" onClick={row.action} className="mt-2 w-full rounded-lg py-1.5 text-[11px] font-semibold" style={{ background: "var(--ai-bg-soft)", color: "var(--ai-primary)" }}>
                          {t("dashboard.insightsApplySuggestion")}
                        </button>
                        <AiCoreBridgeButton prompt={row.coreAiPrompt} className="mt-2" />
                      </div>
                    </div>
                  </div>
                ))}
