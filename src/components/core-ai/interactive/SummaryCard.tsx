...
export function SummaryCard({ title, rows, actions, status }: SummaryCardProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-lg overflow-hidden"
    >
      {/* Header */}
      <div className="px-4 py-2.5 border-b border-[var(--color-border)] flex items-center justify-between">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-[var(--color-textSecondary)]">{title}</h4>
        {status && (
      ...
      <div className="px-4 py-3 space-y-2">
        {rows.map((row, i) => (
          ...
          >
            <span className="text-[var(--color-textSecondary)]">{row.label}</span>
